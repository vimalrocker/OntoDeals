...
namespace Dcupon.Models
{
    public class CuponDetailsModel : CuponDetail
    {
        ...
        public List<SelectListItem> subcatecorylistitem { get; set; }
        ...
        public bool bulkSave(List<CuponDetail> objCuponDetaillist)
        {
            ...
            using (OntoDealsEntities db = new OntoDealsEntities())
            {
                try
                {
                    var Result = (from Coupondetails in db.CuponDetails
                                  where Coupondetails.WebsitesId == 12
                                  select Coupondetails).ToList();
                    if (Result.Count != 0)
                    {
                        foreach (var item in Result)
                        {
                            db.CuponDetails.Remove(item);
                        }

                        db.SaveChanges();
                    }

                    foreach (var item in objCuponDetaillist)
                    {
                        db.CuponDetails.Add(item);
                    }
        ...
        public IEnumerable<SelectListItem> subcategoryList(int id)
        {
            ...
            IEnumerable<SubCategory> scategoryList = scatergoryModel.Load().Where(x=> x.categoriesID==id);
