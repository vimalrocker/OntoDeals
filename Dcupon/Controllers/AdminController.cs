using Dcupon.BusinessRepository;
...
namespace Dcupon.Controllers
{
    public class AdminController : Controller
    {
        ...
        public ActionResult DashBoard(AdminModel model)
        {
            ...
            objadminprofile.Password = Helper.Encrypt(model.Password, true);//"c91zpYPbSHQCEBKrqmaTSg==";// Helper.Encrypt(model.Password, true);  //;
        ...
        [HttpPost]
        public ActionResult Subcategoryfilter(CuponDetailsModel model)
        {
            model.subcatecorylistitem = model.subcategoryList(model.categoryid).ToList();
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult CouponsList(int? id)
        {
            ...
            IEnumerable<DataModel> Listoffiltercupon = model.listofCupon.Where(x => x.CatId == id);
            model.listofCupon = Listoffiltercupon.ToList();
            }
            return View(model);
        ...
        public ActionResult UpdateCupons(int id)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["logedIn"])))
            {
                ...
                model.subcatecorylistitem = model.subcategoryList(model.CategoriesId).ToList();
        ...
        public JsonResult AdminPasswordReset(string email, string token, string newpassword)
        {
            ...
            if (model.ValidateUserEmailtoken(model))
            {
                ...
                if (model.Update(obj))
                {
                    return Json(new { Authentication = "Successful", Message = "Your Password has been reseted successfully" });
                }
                else
                {
        ...
        {
            //Website obj = new Website();
            ApiUploadStatus apiUploadStatus = new ApiUploadStatus();
            // ApiUploadStatus apiuploadistl = new ApiUploadStatus();
            apiUploadStatus.statuslist = apiUploadStatus.ListofStatus();
            return View(apiUploadStatus);
        }



        [HttpPost]
        public ActionResult Groupondata(string dealdata)
        {
            string original = dealdata.Replace("|_|", "&");
            AdminProfile admin = new AdminProfile();

            var jsreader = new JsonTextReader(new StringReader(original));
            var json = (JObject)new JsonSerializer().Deserialize(jsreader);
            BulkDataload obj_Bulkdata = new BulkDataload();
            statusOfloadData = obj_Bulkdata.bulkSave(json);
            return Json(statusOfloadData, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public ActionResult apistatusgrid()
        {
            ApiUploadStatus apiuploadistl = new ApiUploadStatus();
            apiuploadistl.statuslist = apiuploadistl.ListofStatus();
            return PartialView(apiuploadistl);
        }
